...
const storeName = "uwwhitewaterbookstore";
const bookstoreId = "3876";
const storeUrl = "https://www.uwwhitewaterbookstore.com/buy_textbooks.asp?"
...
const headers = {
  ...
  'Host': `www.${storeName}.com`,
  'Pragma': 'no-cache',
  'Referer': `https://www.${storeName}.com/buy_textbooks.asp?`,
...
async function getDept(campusId, termId) {
  try {
    ...
    const response = await fetch(`https://www.${storeName}.com/textbooks_xml.asp?control=campus&campus=${campusId}&term=${termId}`, {
      method: 'GET',
      headers: headers,
    });
...
async function getCourse(deptId, termId) {
  try {
    const response = await fetch(`https://www.${storeName}.com/textbooks_xml.asp?control=department&dept=${deptId}&term=${termId}`, {
      method: 'GET',
      headers: headers,
    });
...
async function getSection(courseId, termId) {
  try {
    const response = await fetch(`https://www.${storeName}.com/textbooks_xml.asp?control=course&course=${courseId}&term=${termId}`, {
      method: 'GET',
      headers: headers,
    });
