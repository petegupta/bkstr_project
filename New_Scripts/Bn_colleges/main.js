...
const storeNames = ["hws"]; //Enter Store name Here in between these two brakes and inverted comma's.

const store_id = "B00192"; // Enter the store store Id here
            ...
            if (!departments) {
              ...
              for (let d = 0; d < departments.length; d++) {
                ...
                if (!courses) {
                  ...
                  // course loop changed
                  for (let cs = 3; cs < courses.length; cs++) {
                      ...
                      for (let s = 0; s < sections.length; s++) {
                        ...
                        if (booksLength > 0) {
                          ...
                          for (let i = 0; i < booksLength; i++) {
                            ...
                            data["bookrow_id"] = " ";
                            data["bookstoreid"] =
                              $("[name=displayStoreId]").attr("value") || "";
...
function createCsv(fullData) {
  ...
  let csvWriter = createCsvWriter({
    path: `./data_csv/bnCollegesArpit_10Apr.csv`,
    header: [
  ...
  console.log(
    `Data uploaded into csv successfully Found Data length "${csvData}" not found data length "${csvDataNotFound}" = ${
      csvData + csvDataNotFound
    }`
  );
  csvWriter
